...
async bookParkingSpot(req, res, next) {
    try {
       const { email = null, parkingSpotID = null, requireToDate = null, requireUntilDate = null, isAutomatic = null } = req.body;
 ...
 async approveOrDeclineReq(req, res, next) {
   try {
      const { email = null, parkingSpotID = null, hostID = null,  requireToDate = null, requireUntilDate = null, answer = null } = req.body;
      ...
      if(answer == "true")
      {
       ...
       await Profiles.updateOne(
         { "email": email },
         {$pull: {'driverWaitingQueue': {parkingId: parkingSpotID, requireToDate: fromTimeDate, requireUntilDate: untilTimeDate}}
         }
       )
       ...
       await Profiles.updateOne(
        ...
        await Profiles.updateOne(
         ...
         {$pull: {'parkingSpots.$.hostWaitingQueue': {bookedBy: email, requireToDate: fromTimeDate, requireUntilDate: untilTimeDate}}
         }
      )
     ...
     else(answer == "false")
     {
        ...
        await Profiles.updateOne(
            {"parkingSpots.parkingId": parkingSpotID},
                 { $push: {'parkingSpots.$.hostDeclineReservations': hostDeclineReservations} }
         )
