 ...
 async searchByLocationAndPriceAndSizeByTime(req, res, next) { //for now works for one futureReservation
   try {
      ...
      allParkingSpots.map(parkingSpot => {
         var schedule = new AvailabilitySchedule(parkingSpot.windowsOfTime.AvailablefromTime, parkingSpot.windowsOfTime.AvailableUntilTime); // Second week of Jan 2017
         parkingSpot.windowsOfTime.map(window => {
            ...
            schedule.addAvailability(window.AvailablefromTime, window.AvailableUntilTime); // Sat Jan 14 12pm-3pm UTC
