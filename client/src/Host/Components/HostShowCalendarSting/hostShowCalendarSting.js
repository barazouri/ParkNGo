...
class HostShowCalendarSting extends React.Component {
  constructor (props) {
    ...
    this.getUrlurlApproveOrDeclineForApi = this.getUrlurlApproveOrDeclineForApi.bind(
      this
    )
  ...
  closePopUp (childData, childDataAnswer) {
    ...
    if (childDataAnswer === true) {
      ...
      console.log(
        'this.state.setUser.requireToDate: ',
        this.state.setUser.requireToDate
      )
      ...
      fetch(`${url}`, {
        ...
        body: `email=${this.state.setUser.bookedBy}&parkingSpotID=${
          this.state.parkingSpot.parkingId
        }&hostID=${1}&requireToDate=${
          this.state.setUser.requireToDate
        }&requireUntilDate=${
          this.state.setUser.requireUntilDate
        }&answer=${true}` // <-- Post parameters
      ...
      this.saveFeedBack()
      let tmpParkingSpot = this.state.parkingSpot
      let indexOfReservation = tmpParkingSpot.hostWaitingQueue.indexOf(
        this.state.setUser
      )
      tmpParkingSpot.futureReservations.push(
        tmpParkingSpot.hostWaitingQueue.splice(indexOfReservation, 1)[0]
      )
      navigation.navigate('HostParkingSpotCalendar', {
        parkingSpot: tmpParkingSpot
      })
    } else if (childDataAnswer === false) {
      // need to add like above but with answer=${false}
      ...
      tmpParkingSpot.hostWaitingQueue.splice(indexOfReservation, 1)
                ...
                <Card>
                  {
                    <View key={i} style={styles.user}>
                      <Text style={styles.name}>
                        Available from: {windowOfTime.AvailablefromTime}
                      </Text>
                      <Text style={styles.name}>
                        Available until: {windowOfTime.AvailableUntilTime}
                      </Text>
                      <Text style={styles.name}>
                        is automatic: {windowOfTime.isAutomatic.toString()}
                      </Text>
                    </View>
                  }
                </Card>
      ...
      return (
        <SafeAreaView style={styles.container}>
          ...
          >
            ...
            {this.state.parkingSpot.futureReservations.map(
              (futureReservation, i) => {
                return (
                  <Card>
                    {
                      <View key={i} style={styles.user}>
                        <Text style={styles.name}>
                          Booked by: {futureReservation.bookedBy}
                        </Text>
                        <Text style={styles.name}>
                          Booked to: {futureReservation.requireToDate}
                        </Text>
                        <Text style={styles.name}>
                          Booked until: {futureReservation.requireUntilDate}
                        </Text>
                      </View>
                    }
                  </Card>
      ...
      return (
        <SafeAreaView style={styles.container}>
          ...
          >
            ...
            {this.state.parkingSpot.hostWaitingQueue.map(
              (hostWaitingQueue, i) => {
                return (
                  <TouchableOpacity
                    onPress={() => this.setUser(hostWaitingQueue)}
                    style={{ left: 10 }}
                  >
                    <Card>
                      {
                        <View key={i} style={styles.user}>
                          <Text style={styles.name}>
                            Requested by: {hostWaitingQueue.bookedBy}
                          </Text>
                          <Text style={styles.name}>
                            Require to: {hostWaitingQueue.requireToDate}
                          </Text>
                          <Text style={styles.name}>
                            Require until: {hostWaitingQueue.requireUntilDate}
                          </Text>
                          <View style={styles.input}>
                            <VerifyPopUp
                              decline={true}
                              dialogVisible={
                                this.state.dialogVisibleAnswerParking
                              }
                              closePopUp={this.closePopUp}
                              subject='Approve or Decline the Request'
                              topTitle='Let the driver know your answer'
                            />
                          </View>
                        </View>
                      }
      ...
      return (
        <SafeAreaView style={styles.container}>
          ...
          >
            ...
            {this.state.parkingSpot.pastReservations.map(
              (pastReservation, i) => {
                return (
                  <Card>
                    {
                      <View key={i} style={styles.user}>
                        <Text style={styles.name}>
                          Booked by: {pastReservation.bookedBy}
                        </Text>
                        <Text style={styles.name}>
                          Booked to: {pastReservation.fromoDate}
                        </Text>
                        <Text style={styles.name}>
                          Booked until: {pastReservation.untilDate}
                        </Text>
                      </View>
                    }
                  </Card>
